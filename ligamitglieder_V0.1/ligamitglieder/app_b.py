...
app.secret_key = hashlib.sha1(str(ran).encode('utf-8')).hexdigest()
...
def login():
    ...
    if request.method == 'POST':
        ...
        password = hashlib.sha1(str(request.form['password']).encode('utf-8')).hexdigest()
...
def home():
    if request.method == 'POST':
        if "old_password" in request.form:
            password = hashlib.sha1(str(request.form['old_password']).encode('utf-8')).hexdigest()
            if password == current_user.passwort and request.form['new_password'] == request.form['confirm_password'] and request.form['new_password'] != '':
                ...
                new_pw = hashlib.sha1(str(request.form['new_password']).encode('utf-8')).hexdigest()
...
def reset():
    if request.method == 'POST':
        ...
        token = hashlib.sha1(str(np.random.randint(999999999999999)).encode('utf-8')).hexdigest()
        token2 = hashlib.sha1(str(np.random.randint(999999999999999)).encode('utf-8')).hexdigest()
    ...
    if user.tokenttl < int(time.time()):
        ...
        if request.method == 'POST':
            if request.form['new_password'] == request.form['confirm_password'] and request.form['new_password'] != '':
                user.passwort = hashlib.sha1(str(request.form['new_password']).encode('utf-8')).hexdigest()
                db.session.commit()
